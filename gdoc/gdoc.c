...
proc_file()
{
	...
	if(g_step && g_html) {
		...
		//printf("  <title>%s%s%s</title>\n", ref_project ? ref_project : "", ref_project && ref_file " - " : "", ref_file ? ref_file : "");
		printf("  <title>");

		if(ref_file) {
			if(ref_project) {
				printf("%s", ref_project);
			}
		...
		printf("    h1 {border-bottom:1px dotted black;background-color:#B0C4DE;}\n");
		printf("    h2 {border-bottom:1px dotted black;background-color:#B0C4DE;margin-top:1.25em;}\n");
		printf("    h3 {border-bottom:1px dotted black;background-color:#B0C4DE;}\n");
		printf("    dt {font-weight:bold;}");
		printf("  </style>\n");
		printf(" <head>\n");
		printf(" <body>\n");
	}
...
reset_tags()
{
	...
	tag_fn = tag_brief = tag_file = tag_section = tag_project = NULL;
...
dump_st1()
{
	...
	if(tag_file) {
		if(tag_project) {
			header(tag_project, 0, NULL);                 // Project
		}

		if(strcmp(tag_file, TAG_FILE_MAIN)) {
			header(*tag_file ? tag_file : g_fname, 1, NULL);  // File
		}
	}
	else if(tag_fn) {

		if(!g_fns) {
			header("Functions Documentation", 1, NULL);
		}

		pch = ref_fnlinks[g_fns++];

		header(tag_fn, 2, pch + 1);                       // Function
	}
	else if(tag_section) {

		if(!g_scs) {
			header("Sections Documentation", 1, NULL);
		}

		pch = ref_scslinks[g_scs++];

		header(tag_section, 2, pch + 1);                  // Section
	}
	else {
		header("Unknown", 2, NULL);                       // Unknown -- FIXME
	}
