/*	NO VALE!!!!!!!!!!!!!!!te_cpc2.c
	...
	CRT module for the Amstrad CPC with CP/M 2.
	...
	The Amstrad CPC with CP/M 2 has specific CRT characters control meaning,
...
/* Definitions
   -----------
*/
#define CRT_NAME "Amstrad CPC & CP/M 2"
...
#define H_1 "Down   ^^ [ABAJO]  Right  ^L [DER.]"
...
CrtSetup()
{
	CrtOut(4); CrtOut(2); /* 24x80 mode on */
	CrtOut(6);            /* Text mode on */
}
...
CrtClear()
{
	CrtOut(12); /* Clear CRT and cursor to 0,0 */
}
...
int row, col;
{
	CrtOut(31); CrtOut(col + 1); CrtOut(row + 1);
}
